...
export default {
    onload: ({ extensionAPI }) => {
        ...
        extensionAPI.ui.commandPalette.addCommand({
            label: "Random cryptic crossword from The Guardian",
            callback: () => {
                const uid = window.roamAlphaAPI.ui.getFocusedBlock()?.["block-uid"];
                if (uid == undefined) {
                    alert("Please focus a block before importing a puzzle");
                    ...
                    window.roamAlphaAPI.updateBlock(
                        { block: { uid: uid, string: "Loading...".toString(), open: true } });
                    fetchCrossword(uid, false, true, false);
                }
            }
        });

        extensionAPI.ui.commandPalette.addCommand({
            label: "Today's cryptic crossword from The Guardian",
            callback: () => {
                const uid = window.roamAlphaAPI.ui.getFocusedBlock()?.["block-uid"];
                if (uid == undefined) {
                    alert("Please focus a block before importing a puzzle");
                    ...
                    window.roamAlphaAPI.updateBlock(
                        { block: { uid: uid, string: "Loading...".toString(), open: true } });
                    fetchCrossword(uid, false, false, true);
                }
            }
        });
        ...
        const args2 = {
            text: "GUARDIANCROSSWORDTODAY",
            help: "Import today's cryptic crossword from The Guardian",
            handler: (context) => {
                window.roamAlphaAPI.updateBlock(
                    { block: { uid: context.triggerUid, string: "Loading...".toString(), open: true } });
                fetchCrossword(context.triggerUid, false, false, true);
            }
        };

        const args3 = {
            text: "GUARDIANCROSSWORD",
            help: "Import a random cryptic crossword from The Guardian",
            handler: (context) => {
                window.roamAlphaAPI.updateBlock(
                    { block: { uid: context.triggerUid, string: "Loading...".toString(), open: true } });
                fetchCrossword(context.triggerUid, false, true, false);
            }
        };
        ...
        Object.keys(localStorage)
            .filter(x =>
                x.startsWith('roam_research_crossword_'))
            .forEach(x =>
                localStorage.removeItem(x));

        async function fetchCrossword(blockUid, today, guardianRandom, guardianToday) {
            ...
            breakme: {
                if (today) { // get today's crossword from XWordInfo
                    ...
                    const response = await fetch(url);
                    data = await response.json();
                } else if (guardianRandom) { // get random guardian crossword from an archive on github
                    var url = `https://frozen-forest-74426-64fa1018c64c.herokuapp.com/getGuardianRandom`;
                    const response = await fetch(url);
                    data = await response.json();
                } else { // get a random NYT crossword from an archive on github
                ...
                if (guardianRandom || guardianToday) {
                    let sourceObject = convertGuardianToSourceObject(data);
                    let sourceString = JSON.stringify(sourceObject);

                    cAuthor = data.creator.name.toString();
                    let milli = data.webPublicationDate;
                    const dateObj = new Date(milli);
                    cDay = String(dateObj.getDate());
                    cMonth = String(dateObj.getMonth() + 1);
                    cYear = String(dateObj.getFullYear());
                    crosswordDate = convertRoamDate(cDay, cMonth, cYear);

                    // setTimeout is needed because sometimes block is left blank
                    setTimeout(async () => {
                        await window.roamAlphaAPI.updateBlock({
                            "block": {
                                "uid": blockUid, "string": "**Guardian Cryptic Crossword**",
                                "props": {
                                    "crossword_definition": sourceString,
                                    "crossword_guesses": "{\"guesses\":{}}"
                                }
                            }
                        });
                        var authorString = "";
                        ...
                        await window.roamAlphaAPI.createBlock({
                            "location": { "parent-uid": blockUid, "order": 0 }, "block": {
                                "string": "[[" + crosswordDate + "]] ~ " + authorString + " ~ No [[" + data.number + "]]"
                            }
                        });
                        await window.roamAlphaAPI.createBlock({
                            "location": { "parent-uid": blockUid, "order": 1 }, "block": {
                                "string": "{{crossword}}"
                            }
                        });
                    }, 200);
                    ...
                    setTimeout(async () => {
                        await window.roamAlphaAPI.updateBlock({
                            "block": {
                                "uid": blockUid, "string": "**NYT Crossword**",
                                "props": {
                                    "crossword_definition": sourceString,
                                    "crossword_guesses": "{\"guesses\":{}}"
                                }
                        ...
                        await window.roamAlphaAPI.createBlock({
                            "location": { "parent-uid": blockUid, "order": 0 }, "block": {
                                "string": "[[" + crosswordDate + "]] ~ " + authorString + "",
                            }
                        });
                        await window.roamAlphaAPI.createBlock({
                            "location": { "parent-uid": blockUid, "order": 1 }, "block": {
...
function convertGuardianToSourceObject(guardianData) {
    const sourceObject = { across: {}, down: {} };

    guardianData.entries.forEach(entry => {
        const direction = entry.direction.toLowerCase();
        if (direction !== 'across' && direction !== 'down') return;

        const clueNumber = entry.humanNumber.toString();
        const position = entry.position;

        sourceObject[direction][clueNumber] = {
            clue: entry.clue.replace(/’/g, "'"), // Normalize apostrophes
            answer: entry.solution,
            row: position.y,  // Guardian uses y for row
            col: position.x   // Guardian uses x for column
        };
    });
...
function convertRoamDate(cDay, cMonth, cYear) {
    let crosswordDate = "";
